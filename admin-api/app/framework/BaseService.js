...
class IndexService extends Service {
   ...
   * @returns
   */
  async executeRawSQL(sql, replacements = []) {
    ...
    try {
      ...
      const results = await this.app.model.query(sql, { type, replacements });
      // 将下划线分隔的字段名转换为驼峰命名
   ...
   * @returns 
   */
  async remove(tableName, whereObj = {}) {
    const where = {};
    Object.keys(whereObj).map((item) => {
      const value = whereObj[item];
      if ({}.toString.call(value) !== "[object Undefined]") {
    ...
    // 使用软删除，更新 isDeleted 字段而不是真正删除记录
    const { accountId } = this.ctx.request.header;
    const updateData = {
      isDeleted: 1,
      updatedBy: accountId,
    ...
    const [affectedCount] = await this.app.model[tableName].update(updateData, { where });
